...
##########################################################################################
            ...
            for car_id in car_bhvr_dict.keys():
                for i in range(0,len(car_bhvr_dict[car_id])):
                    ...
                    if car_velocity > RiskVelocity:
                        RiskLevel[car_id] = RiskLevel_01  # Risk 1_1
                        
                        for ped_id in ped_frame_index_dict.keys():
                            car_frame_idx= car_frame_index_dict[car_id]
                            ped_frame_idx= ped_frame_index_dict[ped_id]
                                
                            if car_frame_idx == ped_frame_idx:
                                RiskLevel[car_id] = RiskLevel_02  # Risk 2_1
                                                
            for car_id in car_frame_index_dict.keys():
                for ped_id in ped_frame_index_dict.keys():
                    sum_x =0
                    sum_y =0
                    car_frame_idx= car_frame_index_dict[car_id]
                    ...
                    if car_frame_idx == ped_frame_idx:
                        ...
                        distance = utils.get_pixel_distance(car_traj_dict[car_id][0][0],car_traj_dict[car_id][0][1],ped_traj_dict[ped_id][0][0],ped_traj_dict[ped_id][0][1]) / R
                        
                        if distance < RiskDistance:
                            RiskLevel[car_id] = RiskLevel_03  #Risk 3_2
                        
                        car_traj_len = len(car_traj_dict[car_id])
                        ped_traj_len = len(ped_traj_dict[ped_id])

                    
                        if car_traj_len > K :
                            cx =  car_traj_dict[car_id][car_traj_len-1][0] - car_traj_dict[car_id][0][0]
                            cy = car_traj_dict[car_id][car_traj_len-1][1] - car_traj_dict[car_id][0][1]
                            car_dx= cx / (car_traj_len*FPS)
                            car_dy= cy / (car_traj_len*FPS)
                        
                        if ped_traj_len > K :
                            px =  ped_traj_dict[ped_id][ped_traj_len-1][0] - ped_traj_dict[ped_id][0][0]
                            py = ped_traj_dict[ped_id][ped_traj_len-1][1] - ped_traj_dict[ped_id][0][1]
                            ped_dx= px / (ped_traj_len*FPS)
                            ped_dy =py / (ped_traj_len*FPS)
                        else:
                            print(" less Risk")
                        
                        #print(ped_dx, ped_dy)
                        car_prediction_1sec = (car_traj_dict[car_id][car_traj_len-1][0] + car_dx, car_traj_dict[car_id][car_traj_len-1][1]+car_dy)            
                        car_prediction_2sec = (car_traj_dict[car_id][car_traj_len-1][0] + car_dx*2, car_traj_dict[car_id][car_traj_len-1][1]+car_dy*2)
                        car_prediction_3sec = (car_traj_dict[car_id][car_traj_len-1][0] + car_dx*3, car_traj_dict[car_id][car_traj_len-1][1]+car_dy*3)
                        
                        ped_prediction_1sec = (ped_traj_dict[ped_id][ped_traj_len-1][0] + ped_dx, ped_traj_dict[ped_id][ped_traj_len-1][1]+ped_dy)            
                        ped_prediction_2sec = (ped_traj_dict[ped_id][ped_traj_len-1][0] + ped_dx*2, ped_traj_dict[ped_id][ped_traj_len-1][1]+ped_dy*2)
                        ped_prediction_3sec = (ped_traj_dict[ped_id][ped_traj_len-1][0] + ped_dx*3, ped_traj_dict[ped_id][ped_traj_len-1][1]+ped_dy*3)
                        ...
                        if car_grid_prdiction[0] == ped_grid_prdiction[0]: # Risk 3_1
                            RiskLevel[car_id] = RiskLevel_03  
                            
                        if car_grid_prdiction[1] == ped_grid_prdiction[1]: # Risk 2_2
                            RiskLevel[car_id] = RiskLevel_02
                            
                        if car_grid_prdiction[2] == ped_grid_prdiction[2]: # Risk 1_2
